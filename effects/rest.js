...
//
console.log('🎥 Rest effect module loading...');
...
(() => {
  ...
  function createOverlay() {
    ...
    console.log('🎥 Creating rest video overlay...');
    ...
    const videoFormats = [
      { file: 'Resting.webm', type: 'video/webm' },
      { file: 'Resting.mp4', type: 'video/mp4' }
    ];
    ...
    video.addEventListener('error', (e) => {
      ...
      errorDiv.textContent = '❌ Video format not supported. Please convert Resting.mp4 to WebM format.';
    ...
    console.log('✅ Rest video overlay created');
  }

  function start() {
    console.log('🎥 Rest effect starting...');
    if (running) return;
    ...
    running = true;
    console.log('✅ Rest effect started');
  }

  function stop() {
    console.log('⏸️ Rest effect stopping...');
    if (!running) return;
    running = false;
    ...
    setTimeout(() => {
      ...
      console.log('✅ Rest effect stopped and cleaned up');
    }, CONFIG.overlayTransition); // Match the transition duration
  }

  function cleanup() {
    console.log('🧹 Cleaning up rest effect...');
    stop();
  }
  ...
  window.RestEffect = {
    ...
    cleanup,
    init: () => {
      console.log('🎬 Rest effect initialized');
    }
  };

  console.log('✅ Rest effect module loaded and exported to window.RestEffect');
})();
